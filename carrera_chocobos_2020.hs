...
bajaVelocidad :: Chocobo -> Int -> Int
bajaVelocidad = (-).velocidad

correrDistancia :: Chocobo -> Int -> Int
correrDistancia chocobo = div (velocidad chocobo)
...
agua :: Int -> Terreno
agua _ chocobo | esChocoboAzul chocobo = velocidad chocobo
agua profundidad chocobo | profundidad < 50 = (`bajaVelocidad` (profundidad*2)) chocobo
agua _ chocobo | otherwise = ((+5).tiempo) chocobo
...
pantano :: Terreno
pantano chocobo = (`bajaVelocidad` 1) chocobo - agua 20 chocobo - bosque chocobo
...
correrTramo :: Tramo -> Chocobo -> Int
correrTramo tramo chocobo = (`correrDistancia` (longitud tramo)) (chocobo { velocidad = velocidad chocobo - (terreno tramo) chocobo})
...
ejemploDePista = [
  ...
  Tramo 5 bosque,
  Tramo 5 asfalto,
  Tramo 5 (\chocobo -> bajaVelocidad chocobo 3) ]
...
masRapidoQueCampeon tramo campeon = length.filter (< tiempo campeon).map (correrTramo tramo)
...
correrPista chocoboInicial = foldl (\chocobo tramo -> chocobo{tiempo = tiempo chocobo + (correrTramo tramo chocobo) }) chocoboInicial
